...
describe(@"Handling network calls", ^{
    ...
    it(@"AvoNetworkCallsHandler builds proper body for schema tracking", ^{
        ...
        expect([actualTrackSchemaBody objectForKey:@"sessionId"]).to.equal(@"");
        expect([actualTrackSchemaBody objectForKey:@"trackingId"]).to.equal(@"");
        expect([actualTrackSchemaBody objectForKey:@"anonymousId"]).toNot.beNil();
