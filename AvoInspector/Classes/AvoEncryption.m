...
+ (NSData * _Nullable)decompressPublicKey:(NSData *)compressedKey {
    ...
    if (tempKey == NULL) {
        ...
        if ([AvoInspector isLogging]) {
            NSLog(@"[avo] Avo Inspector: Compressed public key (0x02/0x03 prefix) is not supported on iOS 13-15. Provide an uncompressed key (65-byte 0x04-prefixed or 64-byte raw X||Y) to support iOS 13-15. Encryption will be skipped.");
        }
        return nil;
    }
    ...
    if (uncompressed == nil) {
        if ([AvoInspector isLogging]) {
            NSLog(@"[avo] Avo Inspector: Failed to export uncompressed public key on iOS 13-15. Provide an uncompressed key (65-byte 0x04-prefixed or 64-byte raw X||Y) to support iOS 13-15. Encryption will be skipped.");
        }
    }
    return uncompressed;
}
